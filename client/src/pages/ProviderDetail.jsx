    ...
    <div className="max-w-6xl mx-auto px-4 py-10 grid lg:grid-cols-[1fr,380px] gap-8 items-start">
      ...
      <div>
        <div className="flex items-start gap-4">
          <div className="w-20 h-20 rounded-2xl bg-pine-100 text-pine-800 font-display font-bold
                          flex items-center justify-center text-2xl shrink-0">
            {initials(provider.user.name)}
          </div>
          <div>
            <h1 className="text-3xl font-bold text-pine-900">{provider.user.name}</h1>
        ...
        <div className="card p-6 mt-6">
          <h2 className="font-bold text-lg text-ink-900">About</h2>
          <p className="mt-2 text-ink-700 leading-relaxed whitespace-pre-line">{provider.bio}</p>
        </div>

        <div className="card p-6 mt-4">
          <h2 className="font-bold text-lg text-ink-900">Services & rates</h2>
          <div className="mt-3 divide-y divide-linen-200">
            {provider.services.map(s => (
              <div key={s.id} className="py-3 flex items-start justify-between gap-4">
                ...
                <p className="font-display font-bold text-lg text-pine-800 whitespace-nowrap">
                  {money(s.hourlyRateCents)}<span className="text-sm text-ink-400 font-sans font-normal">/hr</span>
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="card p-6 mt-4">
          <h2 className="font-bold text-lg text-ink-900">Reviews {provider.reviewCount ? `(${provider.reviewCount})` : ''}</h2>
          {provider.reviews.length === 0 && (
            <p className="mt-2 text-sm text-ink-500">No reviews yet.</p>
          )}
          <div className="mt-3 space-y-4">
            {provider.reviews.map(r => (
              <div key={r.id} className="border-b border-linen-200 last:border-0 pb-4 last:pb-0">
                <div className="flex items-center gap-2">
                  <Stars rating={r.rating} />
                  <span className="text-sm font-semibold text-ink-900">{r.customer.name}</span>
      ...
      <div className="card p-6 lg:sticky lg:top-24">
        {booked ? (
          <div className="text-center py-6">
            ...
            <h2 className="mt-3 font-bold text-xl text-pine-900">Request sent!</h2>
          ...
          <>
            <h2 className="font-bold text-xl text-pine-900">Book {provider.user.name.split(' ')[0]}</h2>
            {!canBook && (
              ...
              <form onSubmit={submit} className="mt-4 space-y-4">
                ...
                {slot && service && (
                  <>
                    ...
                    <div className="flex items-center justify-between rounded-xl bg-linen-100 px-4 py-3">
                      <span className="text-sm font-semibold text-ink-700">Estimated total</span>
                      <span className="font-display font-bold text-xl text-pine-800">
