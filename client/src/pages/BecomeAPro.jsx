...
export default function BecomeAPro() {
  return (
    <div>
      <section className="bg-pine-900 text-linen-50">
        <div className="max-w-6xl mx-auto px-4 py-20 text-center">
          <h1 className="text-4xl md:text-5xl font-bold">
            Turn your skills into <span className="text-sun-400">steady bookings</span>
          </h1>
          <p className="mt-4 text-lg text-linen-200/90 max-w-xl mx-auto">
            Join MyTimeBooked as a pro and get discovered by neighbors who need housekeeping,
            childcare, pet care, yard work, and more.
          </p>
          <Link to="/register?role=PROVIDER" className="btn-accent !px-8 !py-3.5 !text-base mt-8">
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        <h2 className="text-2xl font-bold text-pine-900 text-center">Pros we're looking for</h2>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          {CATEGORIES.map(c => (
            <span key={c.key} className="chip bg-white border-linen-300 text-ink-700 !cursor-default">
              {c.icon} {c.label}
            </span>
          ))}
        </div>
        <div className="text-center mt-10">
          <Link to="/register?role=PROVIDER" className="btn-primary !px-8 !py-3">Create my pro account</Link>
        </div>
      </section>
    </div>
  );
