...
export default function ProviderAvailability() {
  ...
  if (profile === null) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <p className="text-ink-500">Create your <Link to="/pro/profile" className="text-sage-700 font-semibold hover:underline">pro profile</Link> first, then set your availability.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold text-ink-900">Availability</h1>
      ...
      <form onSubmit={add} className="card p-6 mt-6 grid sm:grid-cols-[1fr,auto,auto,auto] gap-3 items-end">
        ...
        <button className="btn-primary !py-3" disabled={busy}>{busy ? 'Adding…' : 'Add window'}</button>
      ...
      <div className="mt-8 space-y-5">
        ...
        {Object.entries(byDay).map(([day, daySlots]) => (
          <div key={day}>
            <h2 className="font-semibold text-ink-900">{day}</h2>
            <div className="mt-2 flex flex-wrap gap-2">
              {daySlots.map(s => (
                <span key={s.id} className="inline-flex items-center gap-2 rounded-full bg-white border border-paper-200 px-4 py-2 text-sm font-medium shadow-sm">
                  {fmtTime(s.start)} – {fmtTime(s.end)}
                  <button onClick={() => remove(s.id)} className="text-ink-400 hover:text-red-500 transition" aria-label="Remove window">✕</button>
                </span>
              ))}
            </div>
          </div>
